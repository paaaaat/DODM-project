...
def solve_vrp(locations, weights, service_times, num_vehicles, vehicle_capacity, t_max, time_windows, restricted_customers):
    ...
    # 2. Force using all available vehicles
    for k in K:
    ...
    # 8. Initial departure from depot
    for k in K:
    ...
    # 12. Customer service restrictions
    ...
    for k in K:
    # Ensure T[k] equals the duration of the route for vehicle k
        model.addConstr(
            T[k] == gp.quicksum(d[j,k] * x[j,0,k] for j in C),
            name=f"exact_duration_{k}"
        )

    # 2. Calculate average duration across all vehicles
    model.addConstr(
        T_avg == gp.quicksum(T[k] for k in K) / num_vehicles,
    ...
    workload_balance_obj = gp.quicksum(dev[k] for k in K) / num_vehicles
    ...
    if model.status == GRB.OPTIMAL:
        ...
        print(f"Total travel time (1st objective): {model.objVal:.2f}")
        print(f"Total workload deviation (2nd objective): {sum(dev[k].X for k in K):.2f}")
        print(f"Mean absolute deviation: {sum(dev[k].X for k in K)/num_vehicles:.2f}")
        ...
        for k in K:
            if any(x[0,j,k].X == 1 for j in C):
                ...
                while True:
                    next_node = None
                    for j in V:
                        if j != current and x[current,j,k].X == 1:
                            next_node = j
                            route.append(j)
                            if j != 0:
        ...
        for k in K:
            max_duration = 0
            for j in C:
                if x[j,0,k].X == 1:  # If j is the last customer before returning to depot
...
if __name__ == "__main__":
    ...
    weights = np.array([30, 30, 50, 70, 35, 45, 32])  # Customer weights
    service_times = np.array([0, 3, 3, 5, 12, 16, 9, 22])  # Depot and customer service times
    ...
    time_windows = [
        ...
        (10, 100),  # Customer 6: [10, 100]
        (20, 120)   # Customer 7: [20, 120]
    ]
    ...
    restricted_customers = [
        ...
        (4, 6, 7)    # Customer 7 cannot be served if 4 and 6 are served by the same
    ]
